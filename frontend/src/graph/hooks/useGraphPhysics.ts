...
export function useGraphPhysics({ nodes, edges, config = {}, onUpdate }: UseGraphPhysicsProps) {
  ...
  const calculateSpringForce = useCallback(
    (distance: number, naturalLength: number) => {
  ...
  const simulateFruchtermanReingold = useCallback(
    (deltaTime: number, currentTemp: number) => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const nodeA = nodes[i];
        if (!nodeA || typeof nodeA === 'string') continue;
        const posA = currentPositions.current.get(nodeA.id);
        if (!posA) continue;

        for (let j = i + 1; j < nodes.length; j++) {
          const nodeB = nodes[j];
          if (!nodeB || typeof nodeB === 'string') continue;
          const posB = currentPositions.current.get(nodeB.id);
          if (!posB) continue;
  ...
  useEffect(() => {
    if (nodes.length > 0) {
      // Starting simulation with nodes
      startSimulation();
    }
