...
export class ContractMetricsCalculator {
    calculateContractMetrics(contract, tests) {
    ...
    calculateOverallMetrics(contracts, tests) {
    ...
    getTestTrends(tests, days = 30) {
        ...
        const firstHalfSuccessRate = firstHalf.reduce((sum, day) => sum + day.passed / Math.max(day.tests, 1), 0) /
            firstHalf.length;
        const secondHalfSuccessRate = secondHalf.reduce((sum, day) => sum + day.passed / Math.max(day.tests, 1), 0) /
            secondHalf.length;
        let trend = 'stable';
    ...
    getContractHealthScore(contract, tests, validations) {
        let score = 100;
        const factors = [];
        ...
        if (contractTests.length === 0) {
            ...
            const successRate = (contractTests.filter(test => test.result === TestResult.PASSED).length /
...
//# sourceMappingURL=contract-metrics.js.map