...
export class CommonDependencies {
  private static readonly DEPENDENCIES: DependencyConfig = {
    framework: [
      {
        name: 'express',
        version: '^4.18.2',
  ...
  static getDependency(name: string): CommonDependency | null {
    for (const category of Object.values(this.DEPENDENCIES)) {
      const dependency = category.find((dep: any) => dep.name === name);
      if (dependency) {
        return dependency;
      }
    }
  ...
  static getRequiredDependencies(): CommonDependency[] {
    const allDependencies: CommonDependency[] = [];
    for (const category of Object.values(this.DEPENDENCIES)) {
      allDependencies.push(...category.filter((dep: any) => dep.required));
    }
  ...
  static getProductionDependencies(): CommonDependency[] {
    const allDependencies: CommonDependency[] = [];
    for (const category of Object.values(this.DEPENDENCIES)) {
      allDependencies.push(...category.filter((dep: any) => dep.type === 'production'));
    }
    return allDependencies;
  }
  ...
  static getDevelopmentDependencies(): CommonDependency[] {
    const allDependencies: CommonDependency[] = [];
    for (const category of Object.values(this.DEPENDENCIES)) {
      allDependencies.push(...category.filter((dep: any) => dep.type === 'development'));
    }
    return allDependencies;
  }
    ...
    for (const [name, version] of Object.entries(serviceDependencies)) {
      ...
      if (commonDep) {
        ...
        if (!this.isVersionCompatible(version, commonDep.version)) {
          issues.push(
            `Incompatible version for ${name}: expected ${commonDep.version}, got ${version}`
          );
        }
      } else {
  ...
  private static isVersionCompatible(actual: string, expected: string): boolean {
