...
export class JWTSecurityValidator {
  ...
  private static readonly RECOMMENDED_SECRET_LENGTH = 128;
  ...
  public static validateJWTConfig(config: JWTConfig): {
    isValid: boolean;
    errors: string[];
    warnings: string[];
  } {
    ...
    if (config.secret && config.secret.length < this.RECOMMENDED_SECRET_LENGTH) {
      warnings.push(
        `JWT secret should be at least ${this.RECOMMENDED_SECRET_LENGTH} characters for production`
      );
    }
  ...
  public static generateSecureSecret(length: number = 128): string {
  ...
  public static isSecretSecure(secret: string): boolean {
  ...
  private static isValidExpirationTime(time: string): boolean {
    const timeRegex = /^(\d+)([smhd])$/;
    return timeRegex.test(time);
  }

  private static parseExpirationTime(time: string): number {
    const timeRegex = /^(\d+)([smhd])$/;
  ...
  public static getSecurityRecommendations(): {
    secret: string[];
    algorithm: string[];
    expiration: string[];
    general: string[];
