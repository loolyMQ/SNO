...
export class KafkaClient {
    ...
    async publish(topic: string, _message: Omit<IKafkaMessage, 'timestamp' | 'source'>): Promise<void> {
        ...
        try {
            await (this.producer as { send(_data: unknown): Promise<unknown> }).send({
                topic,
                messages: [
                    {
    ...
    async subscribe(topics: string[], handlers: Record<string, (_message: IKafkaMessage) => Promise<void>>): Promise<void> {
        if (!this.consumer || !this.isConnected) {
            throw new Error('Kafka consumer not connected');
        }
        try {
            await (this.consumer as { subscribe(_data: unknown): Promise<void> }).subscribe({ topics, fromBeginning: false });
            await (this.consumer as { run(_data: unknown): Promise<void> }).run({
                eachMessage: async ({ topic, partition, message }: { topic: string; partition: number; message: { value?: Buffer } }) => {
                    try {
                        if (!message.value) {
...
export const createKafkaClient = (serviceName: string, brokers?: string[]): KafkaClient => {
    const kafkaBrokers = brokers || process.env['KAFKA_BROKERS']?.split(',') || ['localhost:9092'];
    return new KafkaClient(serviceName, kafkaBrokers);
};
