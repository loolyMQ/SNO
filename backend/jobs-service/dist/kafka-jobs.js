...
import { createKafkaClient, EventTypes, Topics, CommonMiddleware } from '@science-map/shared';
...
const processJob = async (jobId) => {
    ...
    try {
        ...
        if (job.retries >= job.maxRetries) {
            ...
            logger.error(`Job ${job.id} failed permanently: ${error?.message || String(error)}`);
        }
        else {
            job.status = 'pending';
            store.add(job);
            logger.warn(`Job ${job.id} failed, retrying (${job.retries}/${job.maxRetries}): ${error?.message || String(error)}`);
        }
    }
};
...
const eventHandlers = {
    [EventTypes.USER_REGISTERED]: async (message) => {
        try {
            ...
            logger.error(`Error handling USER_REGISTERED: ${error?.message || String(error)}`);
...
const gracefulShutdown = async (signal) => {
    ...
    try {
        ...
        logger.error(`Error during Kafka client disconnect: ${error?.message || String(error)}`);
    }
    setTimeout(() => {
...
(async () => {
    try {
        ...
        logger.error(`Jobs Service startup error: ${error?.message || String(error)}`);
        process.exit(1);
    }
})();
//# sourceMappingURL=kafka-jobs.js.map