...
const scientificPapers: ScientificPaper[] = [
  {
    id: 'paper-1',
...
const eventHandlers: Record<string, (message: IKafkaMessage) => Promise<void>> = {
  [EventTypes.USER_REGISTERED]: async (message: IKafkaMessage) => {
    try {
      ...
      logger.error({ err: error as Error }, '❌ Error handling USER_REGISTERED');
...
app.get('/health', async (_req, res) => {
  try {
    ...
    try {
      ...
      logger.warn({ err: error as Error }, 'MeiliSearch not available');
    }
...
app.get('/search/papers', async (_req, res) => {
  try {
    ...
    const results = scientificPapers.filter(paper => 
    ...
    res.json({
      ...
      logger.error({ err: error as Error }, '❌ Search papers error');
    res.status(500).json({
      success: false,
...
app.get('/search/researchers', async (_req, res) => {
  try {
    ...
    res.json({
      ...
      logger.error({ err: error as Error }, '❌ Search researchers error');
    res.status(500).json({
      success: false,
      error: 'Internal server error',
    });
...
app.get('/search', async (_req, res) => {
  try {
    ...
    const paperResults = scientificPapers.filter(paper => 
      paper.title.toLowerCase().includes(query.toLowerCase()) ||
      paper.abstract.toLowerCase().includes(query.toLowerCase())
    ).slice(0, Math.floor(limit / 2));

    const researcherResults = researchers.filter(researcher => 
      researcher.name.toLowerCase().includes(query.toLowerCase()) ||
      researcher.affiliation.toLowerCase().includes(query.toLowerCase())
    ).slice(0, Math.floor(limit / 2));
    ...
    logger.error({ err: error as Error }, '❌ Universal search error');
    res.status(500).json({
      success: false,
      error: 'Internal server error',
    });
...
app.get('/search/papers/field/:field', (req, res) => {
  ...
  const fieldPapers = scientificPapers.filter(
    paper => paper.field.toLowerCase() === field.toLowerCase()
  );
...
let server: { listen: (port: number, callback?: () => void) => void; close: (callback?: () => void) => void };
...
const gracefulShutdown = async (signal: string) => {
  ...
  try {
    ...
    logger.error({ err: error as Error }, '❌ Error during Kafka client disconnect');
  }
...
(async () => {
  try {
    ...
    logger.error({ err: error as Error }, '❌ Search Service startup error');
    process.exit(1);
  }
})();
