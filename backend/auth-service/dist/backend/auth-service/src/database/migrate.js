import { createPostgreSQLPool } from '@science-map/shared';
...
class MigrationRunner {
    ...
    async getAppliedMigrations() {
        try {
            const result = await this.dbPool.query('SELECT migration_name FROM _prisma_migrations WHERE finished_at IS NOT NULL ORDER BY started_at');
    ...
    async getPendingMigrations() {
        ...
        return allMigrations.filter(migration => !appliedMigrations.includes(migration.name));
    }
    getAllMigrations() {
        const migrations = [];
    ...
    async applyMigration(migration) {
        ...
        try {
            await this.dbPool.transaction(async (client) => {
                await client.query(`INSERT INTO _prisma_migrations (id, checksum, migration_name, started_at, applied_steps_count) 
           VALUES ($1, $2, $3, NOW(), 0)`, [migrationId, checksum, migration.name]);
                await client.query(migration.sql);
                await client.query(`UPDATE _prisma_migrations 
           ...
           WHERE id = $1`, [migrationId]);
            ...
            try {
                await this.dbPool.query(`UPDATE _prisma_migrations 
           SET rolled_back_at = NOW(), logs = $2 
           WHERE id = $1`, [migrationId, JSON.stringify({ error: error.message })]);
            }
            catch (rollbackError) {
